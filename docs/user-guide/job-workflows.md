...
O recurso mais básico do  é a capacidade de executar um ou mais
passos. Esta sequência de passos é chamada de _fluxo de trabalho_.

Os passos do fluxo de trabalho de um  são exibidos ao visualizar o detalhes do 
em uma lista de  ou no formulário do editor de .
...
Os usuários que preferem definir  em formato de texto devem consultar as duas definições de formato:

- XML: [-xml](/user-guide/document-format-reference/-v20.md)
- YAML: [-yaml](/user-guide/document-format-reference/-yaml-v12.md)

Também é possível criar  dentro do console gráfico
e, em seguida, exportar a definição como um arquivo usando o `rd`,
Ferramenta CLI ([rd](https://.github.io/-cli/)), ou via API.

Consulte [Ferramentas de linha de comando e acesso à API](/user-guide/-workflows.html#ferramentas-de-linha-de-comando-e-acesso-a-api).
...
- Execute os passos restantes antes de falhar: Continue para os próximos passos e falhe o  no final.
...
Ao criar uma nova definição de , o formulário de fluxo de trabalho será definido com
...
- _Passos do nó_ opera uma vez em cada _Nó_, o que pode ocorrer várias vezes em um fluxo de trabalho. Para obter uma lista completa de _Passos do nó_, consulte [_Passos do nó_](/user-guide/-plugins.md#passos-do-no)
- _Passos do fluxo de trabalho_ opera apenas uma vez no fluxo de trabalho. Para obter uma lista completa de _Passos do fluxo de trabalho_, consulte [_Passos do fluxo de trabalho_](/user-guide/-plugins.md#passos-do-fluxo-de-trabalho)
...
Depois de liberar o  selecionado, ele pousará na posição desejada
...
inclui scripts, comandos, referências de , etc. (Scripts e comandos devem ter um status de saída de `0` para
sucesso de retorno.)
...
Consulte a seção sobre [Variáveis ​​de contexto](/user-guide/-workflows.html#variaveis-%E2%80%8B%E2%80%8Bde-contexto) para obter mais informações.
...
Botão "Atualizar" se o  estiver sendo modificado.

## Variáveis ​​de Contexto

Quando um passo do  é executado, ele possui um conjunto de variáveis ​​de "contexto" que você pode acessar no passo do . Existem vários conjuntos de variáveis ​​de contexto, incluindo: o contexto  ``, o contexto _Nó_ `node`, e a contexto de Opção `option`.

 variáveis ​​de contexto (escopo global):

- `.name`: Nome do 
- `.group`: Grupo do 
- `.id`: ID do 
- `.execid`: ID da execução atual
- `.executionType`: tipo de execução, pode ser `usuário`, `agendado` ou `agendado pelo usuário` para execuções `Executar  mais tarde`
- `.username`: Nome de usuário do usuário que está executando o 
- `.project`: Nome do projeto
- `.loglevel`: nível de registro, um de: 'ERROR', 'WARN', 'INFO', 'VERBOSE', 'DEBUG'
- `.user.email`: Executando o endereço de e-mail do usuário definido em [Perfil do usuário](/user-guide/10-user.md).
- `.retryAttempt`: Um número que indica a tentativa, se esta execução for uma [nova tentativa](/# nova tentativa).
- `.wasRetry`:`true` se esta execução for uma nova tentativa, caso contrário, `false`. Veja: [repetir](/# nova tentativa).
- `.threadcount`: Threadcount (número de _nós_ executados de uma vez) do 
- `.filter`: O filtro usado para selecionar os _nós_ para este  (se aplicável)
...
- `running.user`: usuário que iniciou o 
...
- `running.argstring`: string de argumento para qualquer opção de 
- `running.project`: nome do projeto
- `running.loglevel`: string de nível de log ('ERROR', 'WARN', 'INFO', 'VERBOSE', 'DEBUG')
Os seguintes valores podem estar disponíveis após o  ser concluído (não disponível para o acionador onstart):
...
- `result.reason`: um código que indica a razão da falha do passo
  ...
  - Strings de código de razão usadas por referências de 
    - `Failed` - o fluxo de trabalho de um  referenciado falhou
    - `NotFound` - referenciado  não encontrado
    - `Unauthorized` - referenciado  não autorizado
    - `InvalidOptions` - opções de entrada  referenciadas inválidas
    - `NoMatchedNodes` - filtros de despacho  _nó_ referenciados não tinham correspondência
...
Variáveis ​​de contexto de opção são referidas como `option.NAME` (mais sobre [ Options](/user-guide/-options.md).)

Pode haver variáveis ​​de contexto adicionais disponíveis, como valores `data.*` Ao usar os [Plugins de filtro de registro de captura de dados](/user-guide/-workflows.html#variaveis-%E2%80%8B%E2%80%8Bde-contexto).

### Uso da variável de contexto

Variáveis ​​de contexto podem ser usadas de algumas maneiras em um passo do , com sintaxes ligeiramente diferentes:

- Comandos, Argumentos de Script e  Argumentos de Referência
...
As variáveis ​​`.*` e `execution.*` mencionadas acima existem no escopo Global.
...
### Ferramentas de linha de comando e acesso à API
 ...
 podem ser exportados ou importados no formato XML ou Yaml usando a API ou a ferramenta CLI `rd`.

- [Exportando ](/api/-api.md#export-s)
- [Importing ](/api/-api.md#importing-s)
- [Ferramenta RD CLI](https://.github.io/wcontrol-cli/)
