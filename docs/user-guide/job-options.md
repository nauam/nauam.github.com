...
O  pode ser configurado para solicitar a entrada de um usuário, definindo uma ou mais opções nomeadas. Uma opção modela um parâmetro nomeado que pode ser obrigatório ou opcional e inclui uma gama de opções que irão serão apresentado ao usuário quando o  for executado.

Os usuários fornecem opções digitando um valor ou selecionando em um menu de opções. Um padrão de validação garante que a entrada está em conformidade com o requisito da opção. Uma vez escolhido, o valor escolhido para a opção fica acessível aos comandos chamados pelo .

As escolhas de opções podem ser modeladas como um conjunto estático ou de uma fonte dinâmica. As escolhas estáticas podem ser modeladas como uma lista separada por vírgulas na definição de . Quando os valores de opção devem ser dinâmicos, o  pode ser definido para usar uma URL para recuperar dados de opção de uma fonte externa. Habilitar  para acessar fontes externas via URL abre a porta para integrar o com outras ferramentas e incorporar seus dados em  workflows.

## Solicitando ao usuário

O efeito óbvio de definir opções de  é sua aparência para o usuário executando o . Os usuários verão uma página chamada "Escolha Opções de execução..." onde as opções de entrada e de menu devem ser configuradas.

Usuários de linha de comando executando  via shell `run`
...
Vale a pena gastar um momento para considerar como as opções se tornam parte da interface do usuário para  e pensar um pouco sobre o próximo nível de formalização do procedimento.
...
- Opções obrigatórias: Tornar uma opção obrigatória significa que o  falhará se um usuário não especificar um valor que não seja em branco. Em outras palavras, um valor em branco ou ausente não é permitido para a opção.
...
Tipos de entrada de opção definem como a opção é apresentada na GUI e como ela é usada quando o  é executado.
...
- "Seguro"
  - uma opção segura que é obscurecida no momento da entrada do usuário e cujo valor não é armazenado no banco de dados. Veja [opções de segurança](/user-guide/-options.md#opcoes-de-seguranca).
- "Autenticação remota segura"
  - uma opção segura que é usada apenas para autenticação remota e não é exposta em scripts ou comandos. Consulte [opções de autenticação remota segura](/user-guide/-options.html#opcoes-de-autenticacao-remota-segura).

## Editor de opções

As opções podem ser criadas para qualquer  armazenado. A página de edição  contém uma área que exibe um resumo das opções existentes e um link para adicionar novas ou editar as existentes.
...
Mover o mouse sobre qualquer linha no editor de opções revela links para excluir ou editar a opção destacada. Pressionar o ícone de remoção exibirá um prompt confirmando que você deseja excluir essa opção do .

Clicar no link "editar" abre um novo formulário que permite modificar todos os aspectos dessa opção.

As opções também podem ser definidas como parte de uma definição de  e posteriormente carregadas no servidor . Consulte [-xml](/user-guide/document-format-reference/-v20.md) e [-yaml](/user-guide/document-format-reference/-yaml-v12.md) e [rd ](/https://.github.io/-cli/commands/#s) páginas se você preferir usar uma definição textual de .
...
Aqui você fornece o nome e a descrição da opção. O nome se torna parte dos argumentos aceitáveis para o , enquanto a descrição será fornecida como texto de ajuda para os usuários que executam o .
...
Define os critérios sobre os quais a entrada deve ser aceita ou apresentada. As escolhas de opções podem ser controladas usando a restrição "Enforced from values". Quando definido como "true", o apresentará apenas um menu pop-up. Se definido como "false", um campo de texto também será apresentado. Insira uma expressão regular no campo "Match Regular Expression" que o  avaliará quando executado.

### Requerimento

Indica se o  só pode ser executado se um valor valido for fornecido para essa opção. Escolha "Não" para indicar que um valor em branco é permitido e escolha "Sim" para indicar que um valor em branco não é permitido.
...
A string delimitadora será usada para separar os vários valores quando o  for executado.
...
Quando estiver satisfeito com a definição da opção, pressione o botão "Salvar" para adicioná-la à definição . Pressionar o botão "Cancelar" descartará as alterações e fechará o formulário.
...
Você pode usar a string `${DATE:format}` como um valor de opção do  (tanto como valor padrão ou como entrada pelo usuário), que será convertido quando o /comando for executado. Esta variável também permite adicionar ou remover dias como você pode ver abaixo.
...
Consulte a seção [Variáveis ​​de contexto](/user-guide/-workflows.md # context-variables).

**Exemplo:**

Um  chamado "hello", tem uma opção chamada "message".

A assinatura da opção "hello"  seria: `-message <>`.
...
Quando o usuário executa o  "hello", será solicitado o
valor da "mensagem".

![Opção inserida](/assets/img/fig0505.png)

Vamos supor que eles escolheram a palavra "howdy" em resposta. A saída do  será:
...
Se você definir a opção como _Required_, o  não será executado, a menos que o usuário forneça um valor que não esteja em branco.
...
Você pode usar o _Valor Padrão_ de uma opção para fornecer um valor caso o usuário não o especifique. Na GUI, o _Valor Padrão_ será automaticamente apresentado na página Execução de . Na CLI ou API, deixando de fora um argumento `-option` para um  usar o valor padrão.
...
Se o  precisa fornecer uma opção em sua forma bruta, sem escape, é possível usar a chave de pesquisa de contexto `unquotedoption` ao invés de `option`.
...
> Considere o exemplo a seguir, onde um usuário tem permissão para fornecer uma lista de diretórios para um  de backup hipotético. Vamos supor também que  processou a lista de diretórios com `tar -zcvf /tmp/archive.tgz @unquotedoption.targetdirs@`. Então, um usuário mal-intencionado pode enganar o  para que ele execute um comando extra fornecendo este valor:
...
Os valores da opção "segura" não são armazenados no banco de dados do quando o  é executado, mas o valor inserido
...
- como tokens de texto simples expandidos em scripts remotos como `@option.name@`.
  - Os passos do fluxo de  de script embutido que contêm uma expansão de token serão expandidas em um arquivo temporário e o arquivo temporário conterá o valor da opção de texto simples.

::: tip DICA
Quando passados ​​como argumentos para  References, eles só podem ser passados ​​como o valor de outra opção Secure. Consulte [Usando opções seguras com referências de ](/user-guide/-options.html#usando-opcoes-seguras-com-referencias-).
:::

### Opções de autenticação remota segura

O [provedor SSH](/Administration/projects/node-execution/ssh.md) integrado para execução de nó permite o uso de senhas para mecanismos de autenticação SSH e/ou Sudo, e as senhas são fornecidas por Opções de autenticação remota segura definidas em um .
...
### Usando opções seguras com referências 

Ao definir um [passo de Referência de  em um fluxo de trabalho](/user-guide/node-steps/builtin.md#passo-de-referencia-de-), você pode especificar os argumentos que são transmitidos a ela. Você pode passar os valores da Opção segura e da Opção de autenticação remota segura de um  de nível superior para uma Referência de , mas os valores de opção _não podem ser passados para outra opção de um tipo diferente_. Portanto, um  pai só pode passar valores de opção para a referência de  se o tipo de opção for o mesmo entre os .
...
Como exemplo, aqui estão dois ,  A e  B, que definem algumas opções:
...
![Caminho de armazenamento para opção segura](/assets/img/s-options-secure-storage-path.png)
...
Isso é útil em alguns cenários quando o é usado para coordenar processos que dependem de outros sistemas:
...
- Qualquer situação na qual variáveis de entrada para seus  devem ser selecionadas a partir de algum conjunto de valores produzidos por um sistema diferente.

## Fornecedor de modelo de opção

O provedor de modelo de opção é um mecanismo que permite que as opções definidas para um  tenham alguns dos valores de entrada possíveis fornecidos por um serviço remoto ou banco de dados.

Os provedores de modelo de opção são configurados por opção (onde um  pode ter zero ou mais opções).
...
4. Por padrão, a resposta HTTP(S) deve incluir o tipo de conteúdo `application/json` no cabeçalho.
   Caso isso não possa ser controlado, o atributo `project.s.disableRemoteOptionJsonCheck` pode ser definido como `true` nas configurações do projeto.

### Configuração

Cada entrada de opção para um  pode ser configurada para obter o conjunto de valores possíveis de um URL remoto. Se você está criando s por meio do [formato de arquivo .xml](/user-guide/document-format-reference/-v20.md#opcao), simplesmente adicione um atributo `valuesUrl` para a` <option> `. Se você estiver modificando o  na interface web do , poderá inserir um URL no campo "URL remoto" para a opção.
por exemplo:
...
::: tip DICA
O esquema de URL do arquivo também é aceitável (por exemplo, `file:/path/to//options/optA.json`).
:::
...
Você pode configurar tempos limites globalmente conforme descrito em [Configuração - Parâmetros de conexão da URL da opção remota do ](/administration/configuration/config-file-reference.md#-config.properties).
...
As opções em cascata permitem que o URL de valores remotos de uma opção incorpore os valores inseridos pelo usuário para outras opções ao executar um . Quando o usuário insere ou seleciona um valor para uma das outras opções, o JSON remoto é atualizado para a opção atual.
...
O URL declarado para "valuesUrl" pode incorporar variáveis que serão preenchidas com certos itens de contexto do  ao fazer a solicitação remota. Isso ajuda a tornar os URLs mais genéricos e contextuais para o .

Dois tipos de expansões estão disponíveis, contexto de  e contexto de opção.

Para incluir as informações do  na URL, especifique uma variável no formato `${.property}`.

Propriedades disponíveis para o contexto do :

- `name`: nome do 
- `group`: Grupo do 
- `description`: descrição do 
- `project`: nome do projeto
- `user.name`: Usuário executando o 
- `.nodename`: Nome do nó do servidor 
- `.serverUUID`: UUID do nó do servidor (modo de cluster)
- `.basedir`: Caminho do arquivo do diretório base do (`file://` URLs apenas)

Além disso, as propriedades `.*` podem ser especificadas sem o prefixo `.`, por exemplo, `${.basedir}`.
...
Para incluir informações de valores de [opção remota em cascata](/user-guide/-options#opcao-remota-em-cascata) no URL, especifique uma variável no formato \${option.name.value}:
...
```http
http://server.com/test?name=${.name}&group=${.group}
```

Transmite o nome e grupo do  como parâmetros de consulta.
...
## Vinculando a  e fornecendo valores de opção

Você pode criar um URL para vincular a um  específico e preencher previamente alguns dos valores de opção adicionando parâmetros de consulta de URL ao URL do .

Formato de parâmetros de consulta para opções:

- `opt.NAME`: fornece um valor para uma opção chamada `NAME`

Por exemplo, se o URL para  for:

```http
http://:4440/project/MyProject//show/ab698597-9753-4e98-bdab-90ebf395b0d0
```
...
```http
http://:4440/project/MyProject//show/ab698597-9753-4e98-bdab-90ebf395b0d0?opt.myopt1=some+value&opt.myotheropt=another+value
```
