...
The 's most basic feature is its ability to execute one or more
_steps_. This sequence of _steps_ is called a _workflow_.

The _steps_ of the  workflow are displayed when viewing a 's
detail from a  listing or within the  editor form.
...
Users preferring to define  in a text format should refer to the two format definitions:

- XML: [-xml](/en/user-guide/document-format-reference/-v20.md)
- YAML: [-yaml](/en/user-guide/document-format-reference/-yaml-v12.md)

It is also possible to author  inside the graphical console
and then export the definition as a file using the
`rd` CLI tool ([rd](https://.github.io/-cli/)), or via the API.
...
- Run remaining _steps_ before failing: Continue to next _steps_ and fail the  at the end.
...
When creating a new  definition, the Workflow form will be set with
...
- _Node_ _Steps_ operate once on each _Node_, which could be multiple times within a workflow. For a full list of _Node_ _Steps_, see [ Plugins - _Node_ _Steps_](/en/user-guide/-plugins.md#_node_-_steps_)
- Workflow _Steps_ operate only once in the workflow. For a full list of Workflow _Steps_, see [Workflow _Steps_](/en/user-guide/-plugins.md#workflow-_steps_)
...
![ _step_ reorder](/assets/img/fig0408.png)

A blue horizontal bar helps highlight the position
where the  will land.

![ _step_ reorder](/assets/img/fig0408a.png)

After releasing the select , it will land in the desired position
...
includes scripts, commands,  references, etc. (Scripts and commands must have an exit status of `0` to
return success.)
...
See the section on [Context Variables](/en/user-guide/-workflows.html#context-variables) for more information.
...
"Update" button if the  is being modified.

## Context Variables

When a  _step_ is executed, it has a set of "context" variables that you can access in the  _step_. There are several sets of context variables, including: the  context ``, the _Node_ context `node`, and the Option context `option`.

 context variables (Global scope):

- `.name`: Name of the 
- `.group`: Group of the 
- `.id`: ID of the 
- `.execid`: ID of the current Execution
- `.executionType` : Execution type, can be `user`, `scheduled` or `user-scheduled` for `Run  Later` executions
- `.username`: Username of the user executing the 
- `.project`: Project name
- `.loglevel`: Logging level, one of: 'ERROR','WARN','INFO','VERBOSE','DEBUG'
- `.user.email`: Executing user's email address set in [User profile](/en/user-guide/10-user.md).
- `.retryAttempt`: A number indicating the attempt, if this execution is a [retry](/en/#retry).
- `.wasRetry`: `true` if this execution is a retry, otherwise `false`. See: [retry](/en/#retry).
- `.threadcount`: Threadcount (number of _nodes_ run at once) of the 
- `.filter`: The filter used to select the _nodes_ for this  (if applicable)
...
- `execution.user`: User who started the 
...
- `execution.argstring`: Argument string for any  options
- `execution.project`: Project name
- `execution.loglevel`: Loglevel string ('ERROR','WARN','INFO','VERBOSE','DEBUG')
The following values may be available after the  is finished (not available for onstart trigger):
...
- `result.reason`: A code indicating the reason the _step_ failed
  ...
  - Reason code strings used by  references
    - `Failed` - referenced  workflow failed
    - `NotFound` - referenced  not found
    - `Unauthorized` - referenced  not authorized
    - `InvalidOptions` - referenced  input options invalid
    - `NoMatchedNodes` - referenced  _node_ dispatch filters had no match
...
Option context variables are referred to as `option.NAME` (more about [ Options](/en/user-guide/-options.md).)

There may be additional context variables available, such as `data.*` values when using the [Data Capture Log Filter Plugins](/en/user-guide/-workflows.html#data-capture-log-filter-plugins).

### Context Variable Usage

Context variables can be used in a few ways in a  _step_, with slightly different syntaxes:

- Commands, Script Arguments and  Reference Arguments
...
The `.*`, and `execution.*` variables mentioned above exist in the Global scope.
...
### Command Line Tools and API access
 ...
 can be exported or imported in XML or Yaml format using the API or the `rd` CLI tool.

- [Exporting ](/en/api/-api.md#exporting-)
- [Importing ](/en/api/-api.md#importing-)
- [RD CLI Tool](https://.github.io/-cli/)
