...
The  can be configured to prompt a user for input by defining one or more named options. An option models a named parameter that can be required or optional and include a range of choices that will be presented to the user when the  is run.

Users supply options by typing in a value or selecting from a menu of choices. A validation pattern ensures input complies to the option requirement. Once chosen, the value chosen for the option is accessible to the commands called by the .

Option choices can be modeled as a static set or from a dynamic source. Static choices can be modeled as a comma separated list in the  definition. When option values must be dynamic, the  can be defined to use a URL to retrieve option data from an external source. Enabling  to access external sources via URL opens the door to integrating with other tools and incorporating their data into  workflows.

## Prompting the user

The obvious effect from defining  options is their appearance to the user running the . Users will be presented a page called "Choose Execution Options..." where input and menu choices must be configured.

Command line users executing  via the `run` shell tool also will specify options as an argument string.

::: warning
The `run` command does not prompt the user for required options so you must specify them directly.
:::

It is worth spending a moment to consider how options become part of the user interface to  and give some thought to this next level of procedure formalization.
...
- Required options: Making an option required means the  will fail if a user does not specify a non-blank value. In other words, a blank or missing value is not allowed for the option.
...
Option Input Types define how the option is presented in the GUI, and how it is used when the  executes.
...
- "Secure"
  - a secure option which is obscured at user input time, and the value of which is not stored in the database. See [secure options](/en/user-guide/-options.md#secure-options).
- "Secure Remote Authentication"
  - a secure option which is used only for remote authentication and is not exposed in scripts or commands. See [secure remote authentication options](/en/user-guide/-options.md#secure-remote-authentication-options).

## Options editor

Options can be created for any stored . The  edit page contains an area displaying a summary to existing options and a link to add new ones or edit existing ones.
...
Moving the mouse over any row in the options editor reveals links to delete or edit the highlighted option. Pressing the remove icon will display a prompt confirming you want to delete that option from the .

Clicking the "edit" link opens a new form that lets you modify all aspects of that option.

Options can also be defined as part of a  definition and later loaded to the server. See [-xml](/en/user-guide/document-format-reference/-v20.md) and [-yaml](/en/user-guide/document-format-reference/-yaml-v12.md) ands [rd ](/en/https://.github.io/-cli/commands/#s) pages if you prefer using an textual  definition.
...
Here you provide the option's name and description. The name becomes part of acceptable arguments to the  while the description will be provided as help text to users running the .
...
A Default Value will automatically be set for the option if it is not otherwise specified by the user, even if not specified among the arguments when executing a  via the command-line or API. Note that a blank value can be specified via the command-line or API, which will override the use of the Default Value.
...
Defines criteria on which input to accept or present. Option choices can be controlled using the "Enforced from values" restriction. When set "true", will only present a popup menu. If set "false", a text field will also be presented. Enter a regular expression in the "Match Regular Expression" field the  will evaluate when run.

### Requirement

Indicates if the  can only run if a non-blank value is provided for that Option. Choose "No" to indicate that a blank value is allowed, and choose "Yes" to indicate that a blank value is not allowed.
...
The delimiter string will be used to separate the multiple values when the  is run.
...
Once satisfied with the option definition, press the "Save" button to add it to the  definition. Pressing the "Cancel" button will dismiss the changes and close the form.
...
You can use the string `${DATE:format}` as a  option value (either as default value or as input by the user), which will be converted when the /command is executed. This variable also allows you to add or remove days as you can see below.
...
See the [Context Variables](/en/user-guide/-workflows.md#context-variables) Section.

**Example:**

A  named "hello" and has an option named "message".

The "hello"  option signature would be: `-message <>`.
...
When the user runs the "hello"  they will be prompted for the
"message" value.

![Option entered](/assets/img/fig0505.png)

Let's assume they chose the word "howdy" in response. The output of the  will be:
...
If you define the option to be _Required_, then the  will fail to run unless the user supplies a value that is not blank.
...
You can use the _Default Value_ of an option to provide a value in the case where the user doesn't specify it. In the GUI, the _Default Value_ will automatically be presented in the  Execution page. From the CLI or API, leaving off a `-option` argument to a  will use the default value.
...
If the  needs to provide an option in its raw, unescaped form it is possible to use the `unquotedoption` context lookup key instead of `option`.
...
> Consider the following example where a user is allowed to supply a list of directories to an hypothetical backup . Let's also suppose the  processed the directory list with `tar -zcvf /tmp/archive.tgz @unquotedoption.targetdirs@`. Then a malicious user could trick the  into running an extra command by supplying this value for:
...
"Secure" option values are not stored in the database when the  is executed, but the value that is entered
...
::: tip
When passed as arguments to  References, they can only be passed as the value of another Secure option. See [Using Secure Options with  References](/en/user-guide/-options.html#using-secure-options-with--references).
:::

### Secure Remote Authentication Options

The built-in [SSH Provider](/en/administration/projects/node-execution/ssh.md) for _node_ execution allows using passwords for SSH and/or Sudo authentication mechanisms, and the passwords are supplied by Secure Remote Authentication Options defined in a .
...
### Using Secure Options with  References

When you [define a  Reference _step_ in a workflow](/en/user-guide/node-steps/builtin.md#-reference-step), you can specify the arguments that are passed to it. You can pass Secure Option values and Secure Remote Authentication Option values from a top-level  to a  Reference, but option values _cannot be passed into another option of a different type_. So a parent  can only pass option values to the  reference if the option type is the same between the .
...
As an example, here is are two ,  A and  B, which define some options:
...
If  A defines a  reference to call  B, then the only valid mapping is shown below:

- plain1 -> plain2
- secure1 -> secure2
- auth1 -> auth2

So the arguments for the  Reference might look like this:

- plain2 ${option.plain1}
- secure2 ${option.secure1}
- auth2 ${option.auth1}

> Note, If you define arguments in the wrong manner, then the Secure and Secure Remote Authentication options will not be set when the  reference is called. Plain options will behave the way they do in Command or Script arguments, and be left as-is as uninterpreted property references.
...
![Storage Path for Secure Option](/assets/img/s-options-secure-storage-path.png)
...
- Any situation in which input variables for your  must be selected from some set of values produced by a different system.

## Option model provider

The Option model provider is a mechanism to allow the Options defined for a  to have some of the possible input values provided by a remote service or database.

Option model providers are configured on a per-Option basis (where a  may have zero or more Options).
...
4. By default, the HTTP(S) response must include the `application/json` content type in the header.
   In case this cannot be controlled, the attribute `project.s.disableRemoteOptionJsonCheck` can be set to `true` in the project settings.

### Configuration

Each Option entry for a  can be configured to get the set of possible values from a remote URL. If you are authoring the  via [.xml file format](/en/user-guide/document-format-reference/-v20.md#option), simply add a `valuesUrl` attribute for the `<option>`. If you are modifying the  in the web GUI, you can entry a URL in the "Remote URL" field for the Option.
...
::: tip
File URL scheme is also acceptable (e.g, `file:/path/to//options/optA.json`).
:::
...
Three styles of return data are supported: simple list, simple object, and a name/value list. For a simple list, the list values will be displayed in a pop-up list when running the . If a simple object or name/value pairs are returned, then the `name` will be displayed in the list, but the `value` will be used as the input.
...
You can configure timeouts globally as described in [Configuration -  Remote Option URL connection parameters](/en/administration/configuration/config-file-reference.md#-config.properties).
...
Cascading options allow an option's Remote values URL to embed the values entered by the user for other options when executing a . When the user enters or selects a value for one of the other options, then the remote JSON is refreshed for the current option.
...
The URL declared for the "valuesUrl" can embed variables which will be filled with certain  context items when making the remote request. This helps make the URLs more generic and contextual to the .

Two types of expansions are available,  context, and Option context.

To include  information in the URL, specify a variable of the form `${.property}`.

Properties available for  context:

- `name`: Name of the 
- `group`: Group of the 
- `description`:  description
- `project`: Project name
- `user.name`: User executing the 
- `.nodename`: Name of the server _node_
- `.serverUUID`: UUID of the server _node_ (cluster mode)
- `.basedir`: File path of the base dir (`file://` URLs only)

Additionally the `.*` properties can be specified without the `.` prefix, e.g. `${.basedir}`.
...
To include [Cascading remote option](/en/user-guide/-options#cascading-remote-options) values information in the URL, specify a variable of the form \${option.name.value}:
...
Passes the  name and group as query parameters.
...
## Linking to  and providing option values

You can create a URL to link to a specific , and pre-fill some of the option values by adding URL query parameters to the 's URL.

Query Parameters format for options:

- `opt.NAME`: provide a value for an option named `NAME`

For example, if the URL for the  is:
